...
(function () {
  ...
  // En Swarm, Traefik enruta /api/* al backend. En desarrollo directo, cambia a http://localhost:8000/api/audit-request
    ...
    o.innerHTML = `
      <div class="audit-login-box" style="position:relative">
        ...
        <div class="audit-login-info">
          ...
          Si aún no las tienes, escríbenos a <a href="mailto:auditor@laconsultoria.cat">auditor@laconsultoria.cat</a> y te avisaremos cuando se abra al público.
        </div>
  ...
  // Bypass admin URL ?admin=ZJKB... — conserva acceso por URL sin UI (auditor)
  function adminBypass() {
    const p = new URLSearchParams(window.location.search);
    return p.get("admin") === "<REDACTED>";
  }
  ...
  form.addEventListener("submit", async function (e) {
    ...
    // Modo soft-launch: sin token y sin bypass → panel de login.
    if (!getToken() && !adminBypass()) {
      buildLoginOverlay();
      return;
    }
    ...
    try {
      ...
      // Modo demo: muestra el payload en consola igual que el script original
      console.log("Audit request payload:", JSON.stringify(payload, null, 2));
      showToast("✓ Solicitud registrada (modo demo). Payload en consola.", "success");
    } finally {
