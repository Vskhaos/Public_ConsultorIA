...
echo "Informe guardado en: $REPORT"
# Enviar archivo como documento a Telegram
curl -s -X POST "https://api.telegram.org/bot$TOKEN_TEL/sendDocument" \
  -F "chat_id=$CHAT_ID_TEL" \
  -F "document=@$REPORT" \
  -F "caption=Informe diario de seguridad - $DATE"
# Dividir el archivo en fragmentos de 1900 caracteres y enviar cada uno
# Enviar como mensaje dividido a Discord
split -C 1800 -d "$REPORT" /tmp/discord_part_

# Enviar cada fragmento de forma segura
for part in /tmp/discord_part_*; do
    if [[ -f "$part" ]]; then
        # Escapar caracteres problemáticos para JSON (comillas, backslashes, saltos)
        MESSAGE=$(sed 's/\\/\\\\/g; s/"/\\"/g' "$part" | awk '{printf "%s\\n", $0}')
        # Enviar mensaje a Discord
        response=$(curl -s -w "%{http_code}" -o /tmp/discord_response.json \
            -H "Content-Type: application/json" \
            -H "Authorization: Bot $TOKEN_DISC" \
            -X POST \
            -d "{\"content\":\"$MESSAGE\"}" \
            "https://discord.com/api/v10/channels/$CHANNEL_ID_DISC/messages")
        # Verificar respuesta de la API
        if [[ "$response" != "200" && "$response" != "204" ]]; then
            echo "⚠️ Error enviando fragmento a Discord (HTTP $response)"
...
rm -f /tmp/discord_part_* /tmp/discord_response.json
echo "✅ Informe enviado a Telegram y Discord correctamente."
